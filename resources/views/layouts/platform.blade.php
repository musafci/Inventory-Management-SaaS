...
<head>
    ...
    <title>{{ $title ?? 'Platform Admin' }} — Oneapp</title>
    <meta name="csrf-token" content="{{ csrf_token() }}">
    @vite(['resources/css/app.css', 'resources/js/app.js'])
    @livewireStyles
</head>
<body class="h-full app-bg" x-data="{ mobileMenu: false, adminName: @js(session('platform_admin_name', '')), adminEmail: @js(session('platform_admin_email', '')) }" x-on:toast.window="$store.toast.add($event.detail.message, $event.detail.type || 'success')">
    <div class="min-h-full">
        ...
        <aside class="sidebar-panel sidebar-transition fixed inset-y-0 left-0 z-50 flex w-72 flex-col lg:hidden"
            ...
            <div class="flex grow flex-col gap-y-2 overflow-y-auto px-5 pb-4">
                @include('components.platform-nav')
            </div>
        </aside>
            ...
            <header class="app-header">
                ...
                <div class="flex flex-1 gap-x-4 self-stretch lg:gap-x-6">
                    <div class="flex items-center gap-3">
                        <span class="hidden rounded-full bg-primary-50 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-primary-700 ring-1 ring-primary-200 sm:inline">Platform</span>
                        <h1 class="text-lg font-bold tracking-tight text-slate-900">{{ $heading ?? 'Dashboard' }}</h1>
                    </div>
                    <div class="flex flex-1"></div>
                    <div class="flex items-center gap-x-3 lg:gap-x-4">
                        <a href="/login" class="org-selector-btn hidden sm:inline-flex">
                        ...
                        <div class="relative" x-data="{ open: false }">
                            <button type="button" class="flex items-center gap-x-2 rounded-xl px-2 py-1.5 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-100" @click="open = !open">
                                <span class="inline-flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-primary-500 to-primary-700 text-sm font-bold text-white shadow-sm" x-text="adminName.charAt(0).toUpperCase()"></span>
                                <span class="hidden sm:inline" x-text="adminName"></span>
                            ...
                            <div x-show="open" x-cloak @click.away="open = false" x-transition class="dropdown-panel w-52">
                                <div class="border-b border-slate-100 px-4 py-3">
                                    <p class="text-sm font-semibold text-slate-900" x-text="adminName"></p>
                                    <p class="text-xs text-slate-500" x-text="adminEmail"></p>
                                    <p class="mt-1 text-[10px] font-semibold uppercase tracking-wider text-primary-600">Super Admin</p>
                                </div>
    ...
    <div x-data x-show="$store.confirm.show" x-cloak class="fixed inset-0 z-[200] flex items-center justify-center p-4"
        ...
        <div class="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl ring-1 ring-slate-900/5"
            ...
            <div class="mt-6 flex justify-end gap-3">
                <button type="button" @click="$store.confirm.deny()" class="btn-secondary" x-text="$store.confirm.denyLabel"></button>
                <button type="button" @click="$store.confirm.confirm()" :class="$store.confirm.type === 'danger' ? 'btn-danger' : ($store.confirm.type === 'warning' ? 'btn-warning' : 'btn-primary')" x-text="$store.confirm.confirmLabel"></button>
            </div>
        </div>
    </div>
