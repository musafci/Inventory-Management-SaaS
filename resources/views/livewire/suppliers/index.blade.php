    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex-1 max-w-lg">
            <div class="relative">
                ...
                <input type="text" wire:model.live.debounce.300ms="search" class="form-input pl-10" placeholder="Search suppliers..." wire:loading.class="search-loading" wire:target="search">
            </div>
        </div>
                ...
                <tbody class="divide-y divide-gray-200 bg-white" wire:transition.opacity.duration.300ms>
                    @forelse($items as $item)
                        <tr class="table-row-hover">
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{{ $item['contact_person'] ?? '-' }}</td>
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-right text-sm">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <button @click="$store.confirm.open('Delete Supplier', 'Are you sure you want to delete this supplier? This action cannot be undone.', 'danger', () => $wire.destroy({{ $item['id'] }}), 'Delete')" class="text-gray-400 hover:text-red-600 transition-colors" title="Delete">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center">
                                ...
                                <button wire:click="openModal()" class="mt-3 btn-primary text-sm">Add Supplier</button>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
                ...
                <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg"
                    ...
                    <form wire:submit.prevent="{{ $editingId ? 'update' : 'store' }}">
                        <div class="border-b border-gray-100 px-6 py-4">
                            <h3 class="text-lg font-semibold text-gray-900">{{ $editingId ? 'Edit Supplier' : 'Add Supplier' }}</h3>
                        </div>
            ...
            <div class="flex min-h-full items-end justify-center px-4 pb-4 pt-4 sm:items-center sm:p-0">
                ...
                <div class="relative w-full max-w-lg transform overflow-hidden rounded-2xl bg-white text-left shadow-xl transition-all sm:my-8">
                    <form wire:submit.prevent="importCsv">
                        <div class="border-b border-gray-100 px-6 py-4">
                            <h3 class="text-lg font-semibold text-gray-900">Import Suppliers from CSV</h3>
