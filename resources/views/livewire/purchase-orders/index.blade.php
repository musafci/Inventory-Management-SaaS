...
@else
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <x-list-search wire:model.live.debounce.300ms="search" placeholder="Search purchase orders..." />
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50/80">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">PO Number</th>
                ...
                <tbody class="divide-y divide-slate-100 bg-white" wire:transition.opacity.duration.300ms>
                    @forelse($items as $item)
                        <tr class="table-row-hover">
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-sm text-slate-500">{{ $item['supplier']['name'] ?? '-' }}</td>
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-right text-sm">
                                <div class="flex items-center justify-end gap-1">
                                    <a href="/purchase-orders/{{ $item['id'] }}/print?print=1" target="_blank" class="action-btn" title="Print">
                                    ...
                                    @if(($item['status'] ?? '') === 'draft')
                                        ...
                                        <button @click.prevent="$store.confirm.open('Send Purchase Order', 'Send this purchase order to the supplier?', 'warning', () => $wire.send({{ $item['id'] }}), 'Send')" class="action-btn action-btn-primary" title="Send">
                                    ...
                                    @if(in_array(($item['status'] ?? ''), ['sent', 'partially_received']))
                                        <button wire:click="openReceiveModal({{ $item['id'] }})" class="action-btn action-btn-success" title="Receive Goods">
                                    ...
                                    @if(in_array(($item['status'] ?? ''), ['draft', 'sent']))
                                        <button @click.prevent="$store.confirm.open('Cancel Order', 'Cancel this purchase order?', 'danger', () => $wire.cancel({{ $item['id'] }}), 'Cancel Order')" class="action-btn action-btn-danger" title="Cancel">
                                    ...
                                    @endif
                                    @if(($item['status'] ?? '') === 'draft')
                                        <button @click.prevent="$store.confirm.open('Delete Order', 'Delete this purchase order? This action cannot be undone.', 'danger', () => $wire.destroy({{ $item['id'] }}), 'Delete')" class="action-btn action-btn-danger" title="Delete">
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-12 text-center">
                                ...
                                <p class="mt-2 text-sm text-slate-500">No purchase orders found.</p>
                ...
                <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-3xl">
                    <form wire:submit.prevent="{{ $editingId ? 'update' : 'store' }}">
                        <div class="border-b border-slate-100 px-6 py-4">
                            <h3 class="text-lg font-semibold text-slate-900">{{ $editingId ? 'Edit Purchase Order' : 'New Purchase Order' }}</h3>
                        </div>
                                    ...
                                    @forelse($form['items'] as $index => $item)
                                        <div class="flex items-start gap-3 rounded-lg border border-slate-200 p-3 bg-slate-50">
                                            <div class="flex-1 grid grid-cols-2 gap-3 sm:grid-cols-4">
                                                ...
                                                <div>
                                                    <label class="text-xs font-medium text-slate-500 mb-1 block">Unit Cost</label>
    ...
    @if($showReceiveModal)
        <div class="fixed inset-0 z-50 overflow-y-auto" x-data x-cloak>
            <div class="flex min-h-full items-end justify-center px-4 pb-4 pt-4 sm:items-center sm:p-0">
                <div class="fixed inset-0 bg-slate-900/50 transition-opacity" wire:click="closeReceiveModal()"></div>
                <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                    <form wire:submit.prevent="submitReceive">
                        <div class="border-b border-slate-100 px-6 py-4">
                            <h3 class="text-lg font-semibold text-slate-900">Receive Goods</h3>
                        </div>
                        <div class="px-6 py-4 space-y-3 max-h-[60vh] overflow-y-auto">
                            @forelse($receiveItems as $index => $rItem)
                                <div class="flex items-center gap-3 rounded-lg border border-slate-200 p-3">
                                    <div class="flex-1">
                                        <p class="text-sm font-medium text-slate-900">Item #{{ $rItem['purchase_order_item_id'] }}</p>
                                    </div>
                                    <div>
                                        <label class="text-xs text-slate-500">Qty to Receive</label>
                                        <input type="number" wire:model="receiveItems.{{ $index }}.quantity" class="form-input text-sm w-24" min="1">
                                    </div>
                                </div>
                            @empty
                                <p class="text-sm text-slate-500 text-center py-4">No items to receive.</p>
                            @endforelse
                            <div>
