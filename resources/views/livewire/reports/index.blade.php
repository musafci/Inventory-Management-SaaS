<div>
    ...
    @if($activeTab === 'stock-valuation')
        <div class="card overflow-hidden">
            <div class="border-b border-slate-100 px-6 py-4">
                <h3 class="text-base font-semibold text-slate-900">Stock Valuation Report</h3>
            </div>
                ...
                <table class="min-w-full divide-y divide-slate-200">
                    <thead class="bg-slate-50/80">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">Unit Cost</th>
                            <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">Total Value</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @forelse($stockValuation as $item)
                            <tr class="table-row-hover">
                                ...
                                <td class="whitespace-nowrap px-6 py-4 text-sm text-slate-900">{{ $item['quantity'] ?? 0 }}</td>
                                <td class="whitespace-nowrap px-6 py-4 text-sm text-slate-900">${{ number_format($item['unit_cost'] ?? 0, 2) }}</td>
                                <td class="whitespace-nowrap px-6 py-4 text-sm font-semibold text-slate-900">${{ number_format($item['total_value'] ?? 0, 2) }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    ...
                                    <p class="mt-2 text-sm text-slate-500">No stock valuation data available.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                    @if(!empty($stockValuation))
                        <tfoot class="bg-slate-50/80 border-t border-slate-200">
                            <tr>
                                <td colspan="4" class="px-6 py-4 text-sm font-semibold text-slate-900">Total Valuation</td>
                                <td class="px-6 py-4 text-sm font-bold text-slate-900">${{ number_format(collect($stockValuation)->sum('total_value'), 2) }}</td>
                            </tr>
                        </tfoot>
                    @endif
                </table>
    ...
    @if($activeTab === 'low-stock')
        <div class="card overflow-hidden">
            <div class="border-b border-slate-100 px-6 py-4">
                <h3 class="text-base font-semibold text-slate-900">Low Stock Report</h3>
            </div>
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-200">
                    <thead class="bg-slate-50/80">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">Product</th>
                            <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">Current Quantity</th>
                            <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">Reorder Point</th>
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @forelse($lowStock as $item)
                            ...
                            <tr class="table-row-hover">
                                <td class="whitespace-nowrap px-6 py-4 text-sm font-medium text-slate-900">{{ $item['product']['name'] ?? '-' }}</td>
                                <td class="whitespace-nowrap px-6 py-4 text-sm {{ $isOut ? 'font-semibold text-red-600' : 'text-slate-900' }}">{{ $qty }}</td>
                                <td class="whitespace-nowrap px-6 py-4 text-sm text-slate-500">{{ $reorder }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-12 text-center">
                                    ...
                                    <p class="mt-2 text-sm text-slate-500">All stock levels look good! No low stock items found.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            <div class="card p-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-slate-900">{{ number_format($summary['total_orders'] ?? 0) }}</p>
                    </div>
                </div>
            </div>
            <div class="card p-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-slate-900">${{ number_format($summary['total_revenue'] ?? 0, 2) }}</p>
                    </div>
                </div>
            </div>
            <div class="card p-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-slate-900">${{ number_format($summary['average_order_value'] ?? $summary['avg_order'] ?? 0, 2) }}</p>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white">
                            @foreach($summary['recent_orders'] as $order)
                                <tr class="table-row-hover">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4 text-sm text-slate-500">{{ $order['customer']['name'] ?? '-' }}</td>
                                    <td class="whitespace-nowrap px-6 py-4 text-sm text-slate-500">{{ $order['order_date'] ?? '-' }}</td>
                                    <td class="whitespace-nowrap px-6 py-4 text-sm font-medium text-slate-900">${{ number_format($order['total'] ?? 0, 2) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
    ...
    @if($activeTab === 'purchase-summary')
            ...
            <div class="card p-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-slate-900">${{ number_format($summary['total_spent'] ?? $summary['total_amount'] ?? 0, 2) }}</p>
                    </div>
                </div>
            </div>
            <div class="card p-6">
        ...
        @if(isset($summary['recent_orders']) && count($summary['recent_orders']) > 0)
            <div class="card overflow-hidden">
                <div class="border-b border-slate-100 px-6 py-4">
                    <h3 class="text-base font-semibold text-slate-900">Recent Purchase Orders</h3>
                </div>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead class="bg-slate-50/80">
                            <tr>
                                <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">PO #</th>
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white">
                            @foreach($summary['recent_orders'] as $order)
                                <tr class="table-row-hover">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4 text-sm text-slate-500">{{ $order['supplier']['name'] ?? '-' }}</td>
                                    <td class="whitespace-nowrap px-6 py-4 text-sm text-slate-500">{{ $order['order_date'] ?? '-' }}</td>
                                    <td class="whitespace-nowrap px-6 py-4 text-sm font-medium text-slate-900">${{ number_format($order['total'] ?? 0, 2) }}</td>
                                </tr>
                            @endforeach
