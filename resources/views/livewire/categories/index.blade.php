    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex-1 max-w-lg">
            <div class="relative">
                ...
                <input type="text" wire:model.live.debounce.300ms="search" class="form-input pl-10" placeholder="Search categories..." wire:loading.class="search-loading" wire:target="search">
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th wire:click="sortBy('slug')" class="cursor-pointer px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 hover:text-gray-700">
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Parent</th>
                ...
                <tbody class="divide-y divide-gray-200 bg-white" wire:transition.opacity.duration.300ms>
                    @forelse($items as $item)
                        <tr class="table-row-hover">
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{{ $item['slug'] ?? '-' }}</td>
                            <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{{ $item['parent']['name'] ?? '-' }}</td>
                            <td class="whitespace-nowrap px-6 py-4 text-right text-sm">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <button @click="$store.confirm.open('Delete Category', 'Are you sure you want to delete this category? This action cannot be undone.', 'danger', () => $wire.destroy({{ $item['id'] }}), 'Delete')" class="text-gray-400 hover:text-red-600 transition-colors" title="Delete">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center">
                                ...
                                <button wire:click="openModal()" class="mt-3 btn-primary text-sm">Add Category</button>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
                ...
                <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-md"
                    ...
                    <form wire:submit.prevent="{{ $editingId ? 'update' : 'store' }}">
                        <div class="border-b border-gray-100 px-6 py-4">
                            <h3 class="text-lg font-semibold text-gray-900">{{ $editingId ? 'Edit Category' : 'Add Category' }}</h3>
                        </div>
