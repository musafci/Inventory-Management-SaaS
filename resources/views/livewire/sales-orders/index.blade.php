...
@else
                ...
                <tbody class="divide-y divide-gray-200 bg-white" wire:transition.opacity.duration.300ms>
                    @forelse($items as $item)
                        <tr class="table-row-hover">
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{{ $item['customer']['name'] ?? '-' }}</td>
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-right text-sm">
                                <div class="flex items-center justify-end gap-1">
                                    <a href="/sales-orders/{{ $item['id'] }}/print?print=1" target="_blank" class="text-gray-400 hover:text-slate-700 transition-colors" title="Print">
                                    ...
                                    @if(($item['status'] ?? '') === 'shipped')
                                        <button @click.prevent="$store.confirm.open('Mark Delivered', 'Mark this order as delivered?', 'info', () => $wire.deliverOrder({{ $item['id'] }}), 'Delivered')" class="text-gray-400 hover:text-emerald-600 transition-colors" title="Mark Delivered">
                                    ...
                                    @if(in_array(($item['status'] ?? ''), ['shipped', 'delivered']))
                                        <button wire:click="openRefundModal({{ $item['id'] }})" class="text-gray-400 hover:text-orange-600 transition-colors" title="Refund">
                                    ...
                                    @if(in_array(($item['status'] ?? ''), ['draft', 'confirmed']))
                                        <button @click.prevent="$store.confirm.open('Cancel Order', 'Cancel this sales order?', 'danger', () => $wire.cancel({{ $item['id'] }}), 'Cancel Order')" class="text-gray-400 hover:text-red-600 transition-colors" title="Cancel">
                                    ...
                                    @endif
                                    @if(($item['status'] ?? '') === 'draft')
                                        <button @click.prevent="$store.confirm.open('Delete Order', 'Delete this sales order? This action cannot be undone.', 'danger', () => $wire.destroy({{ $item['id'] }}), 'Delete')" class="text-gray-400 hover:text-red-600 transition-colors" title="Delete">
                ...
                <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-3xl">
                    <form wire:submit.prevent="{{ $editingId ? 'update' : 'store' }}">
                        <div class="border-b border-gray-100 px-6 py-4">
                            <h3 class="text-lg font-semibold text-gray-900">{{ $editingId ? 'Edit Sales Order' : 'New Sales Order' }}</h3>
                        </div>
    ...
    @if($showFulfillModal)
        <div class="fixed inset-0 z-50 overflow-y-auto" x-data x-cloak>
            <div class="flex min-h-full items-end justify-center px-4 pb-4 pt-4 sm:items-center sm:p-0">
                <div class="fixed inset-0 bg-gray-900/50 transition-opacity" wire:click="closeFulfillModal()"></div>
                <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                    <form wire:submit.prevent="submitFulfill">
                        <div class="border-b border-gray-100 px-6 py-4">
                            <h3 class="text-lg font-semibold text-gray-900">Fulfill Order</h3>
                        </div>
                        <div class="px-6 py-4 space-y-3 max-h-[60vh] overflow-y-auto">
                            @forelse($fulfillItems as $index => $fItem)
                                <div class="flex items-center gap-3 rounded-lg border border-gray-200 p-3">
                                    <div class="flex-1">
                                        <p class="text-sm font-medium text-gray-900">Item #{{ $fItem['sales_order_item_id'] }}</p>
                                    </div>
                                    <div>
                                        <label class="text-xs text-gray-500">Qty to Ship</label>
                                        <input type="number" wire:model="fulfillItems.{{ $index }}.quantity" class="form-input text-sm w-24" min="1">
                                    </div>
                                </div>
                            @empty
                                <p class="text-sm text-gray-500 text-center py-4">No items to fulfill.</p>
                            @endforelse
                            <div>
    ...
    @if($showRefundModal)
        <div class="fixed inset-0 z-50 overflow-y-auto" x-data x-cloak>
            <div class="flex min-h-full items-end justify-center px-4 pb-4 pt-4 sm:items-center sm:p-0">
                <div class="fixed inset-0 bg-gray-900/50 transition-opacity" wire:click="closeRefundModal()"></div>
                <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                            ...
                            @if(count($refundItems) > 0)
                                <div>
                                    ...
                                    @foreach($refundItems as $index => $rItem)
                                        <div class="flex items-center gap-3 rounded-lg border border-gray-200 p-2 mt-2">
                                            <span class="text-sm text-gray-700 flex-1">Item #{{ $rItem['sales_order_item_id'] }}</span>
